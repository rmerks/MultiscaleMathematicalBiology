...
class PygameEngine:
    ...
    def update_rectangles(self, rectangles):
        ...
        self.counter += 1
        if self.counter % 4 == 0:
            view = self._pygame.surfarray.array2d(self.__screen)
            self.frames.append(view.transpose([1, 0]))
    ...
    def save_gif(self):
      print("Saving GIF...")
      with imageio.get_writer("CA.gif", mode="I") as writer:
...
class CAsimulator:
    def __init__(self,
        ...
        """
        #super().__init__(*args, **kwargs)
        self._cellular_automaton = CellularAutomaton()
    ...
    def set_random(self, value):
        self._cellular_automaton.set_random(value)

    def set_neighborhood(self, nbhstring):
        self._cellular_automaton.set_neighborhood(nbhstring)

    def run(self,
            evolutions_per_second=40,
        ...
        :param max_steps:     0 = infinite | > 0 evolution step at which this method will stop
        Warning: is blocking until finished
        """
        print("Running simulation...")
        with contextlib.suppress(KeyboardInterrupt):
            while self._is_not_user_terminated() and self._not_at_the_end(max_steps):
    ...
    def _not_at_the_end(self, max_steps):
        return (self._cellular_automaton.evolution_step < max_steps or max_steps <= 0) \
    ...
    def print_process_info(self, evolve_duration, draw_duration, evolution_step):
        ...
        self.__draw_engine.write_text((10, 5), "Calc time: " + "{0:.4f}".format(evolve_duration) + "s")
